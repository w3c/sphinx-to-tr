...
class TableOfContents {
  ...
  updateAnchor (document, a, page) {
    if (!a.parentElement.tagName.startsWith('H'))
      throw (`unrecognized anchor structure in ${page}: ${a.parentElement.outerHTML}`)
     ...
     */
    const fragment = h.tagName === 'H1' ? '' : a.getAttribute('href')
    const relStr = page + fragment
    if (!this.urlStr2secNo.has(relStr)) {
      // console.log(`unindexed: ${page}/${a.parentElement.outerHTML}`)
      return null
    }
    ...
    h.prepend(bdi)
    // console.warn(`${page}: added ${secNo} to ${h.outerHTML}`)
    return h
  }
}
